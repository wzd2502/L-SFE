...
from .LLMScore import LLMScore_pgmpy
from .BICScore import BICScore_pgmpy
from .otherScore import cv_general, marginal_general


class hc_llm(StructureEstimator):
    """
    ...
    def __init__(self, data, use_cache=True, **kwargs):
        ...
        super(hc_llm, self).__init__(data, **kwargs)
    ...
    def estimate(
        self,
        scoring_method="bicscore", ### wzd: i have change the lambda in bicscore to 2 in pgmpy.
        start_dag=None,
        fixed_edges=set(),
        tabu_length=100,
        max_indegree=None,
        ...
        supported_methods = {
            ...
            'cv-general': cv_general,
            'margin-general': marginal_general,

            
            'llmscore': LLMScore_pgmpy,
            'bicscore-std': BICScore_pgmpy
        }
        if (
            (
        ...
        if isinstance(scoring_method, str):
            if scoring_method == 'bicscore-std':
                score = supported_methods[scoring_method.lower()](data=self.data, data_type = data_type)
            elif scoring_method == 'llmscore':
                score = supported_methods[scoring_method.lower()](data = self.data, score_path = llm_path)
            else:
