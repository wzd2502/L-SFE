...
class TetradSearch:
    ...
    def use_bdeu(self, sample_prior=1, structure_prior=0):
        self.params.set(Params.PRIOR_EQUIVALENT_SAMPLE_SIZE, sample_prior)
        self.params.set(Params.STRUCTURE_PRIOR, structure_prior)
        self.SCORE = score_.BdeuScore()
    
    def use_bic(self,structure_prior=0):
        self.params.set(Params.STRUCTURE_PRIOR, structure_prior)
        self.SCORE = score_.DiscreteBicScore()
    ...
    def run_pc(self, conflict_rule=1, depth=-1, stable_fas=True, guarantee_cpdag=True):
    ...
    def run_dagma(self, lambda1=0.05, w_threshold=0.1, cpdag=False):
        alg = dag.Dagma()
    ...
    def get_dag(self, java=None):
        ...
        n_nodes = dag.getNumNodes()
        nodes = dag.getNodes()
        matrix = np.zeros((n_nodes, n_nodes), dtype=np.int8)

        for edge in dag.getEdges():
